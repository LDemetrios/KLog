...
package expression.withLogging

import logger.*
import java.lang.StringBuilder
import expression.DoubleFunction

fun main(args: Array<String>) = staticScope(args) {
...
class ExpressionParser(val source: String) {
    ...
    fun parseExpression(): DoubleFunction = scope {
        skipWs()
        val res = parseExpr()
    ...
    private fun parseExpr(): DoubleFunction = scope {
        var res = parseTerm()
        skipWs()
        while (test('+', '-')) {
    ...
    private fun parseTerm(): DoubleFunction = scope {
        var res = parseFactor()
        skipWs()
        while (test('*', '/')) {
    ...
    private fun parseFactor(): DoubleFunction = scope {
    ...
    private fun parseClause(): DoubleFunction = scope {
        skipWs()
        if (take('(')) {
            val res = parseExpr()
            assert(')')
    ...
    private fun takeNumber(): Double = scope { //minuses are processed automatically
    ...
    private fun takeWord(): String = scope {
        val sb = StringBuilder()
